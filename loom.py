...
def goto():
    ...
    chars = "/—\|"
    #RAWS
    list_rawK = ["DISCORD", "CODE::BLOCKS", "PROJECT REALITY BF2", "BLOXSTRAP", "STEAM", "VISUAL STUDIO 2022", "VISUAL STUDIO CODE", "MICROSOFT EDGE", "WHATSAPP", "GAGO :3", "LIBREOFFICE", "LIBREOFFICE WRITTER", "VIM", "GVIM", "YOUTUBE","SIGAA"]
    list_rawKC = ["QUIT", "WINDOWS EXPLORER", "WINDOWS CONFIGURATIONS", "EXECUTE (win + r)", "LOOM KEYS LIST", "URL ACESS", "GO TO THE LOOM MENU"]
    #KEYS
    list_keys = ["disc", "cdblocks", "pr", "roblox", "steam", "vs", "vsc", "micro", "wpp", "gago", "lboffice", "lboffice%wr", "vim", "gvim", "youtube", "sigaa"]
    ...
    #SIGAA Key
        ...
        while key == 'sigaa':
            for char in chars:
                sys.stdout.write('\r'+'Initializing SIGAA...'+char)
                time.sleep(0.5)
                sys.stdout.flush()
                import keys.sigaa
            os.system('cls')
            template_loom()
            print("Initialization of SIGAA - Process Completed!")
            break
    #KEYSBASICEND
    #COMMAND KEYS ONLY
    #Command Key :q
